...
class Throwaway(object):
    ...
    def __repr__(self):
        return u'<Throwaway[{0}]>'.format(self.get_email_address())
    ...
    def get_mailbox(self, email=None, email_hash=None):
        ...
        url = 'https://{0}/request/mail/id/{md5}/'.format(self.api_domain, email_hash)
        req = urllib.request.urlopen(url, {"X-Mashape-Key": 12,
                                            "Accept": "application/json"})
        return req.json()